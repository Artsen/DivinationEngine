...
test('Elder Futhark draws three unique persisted runes with historical layers', async ({ page }) => {
  ...
  await expect(connections.getByText('Younger Futhark', { exact: true })).toHaveCount(1)
  const poemWitnesses = cards.first().locator('.poem-witness')
  await expect(poemWitnesses).toHaveCount(3)
  for (let index = 0; index < 3; index += 1) await expect(poemWitnesses.nth(index)).not.toHaveAttribute('open', '')
  ...
  await expect(poemWitnesses.nth(1)).not.toHaveAttribute('open', '')
  await expect(poemWitnesses.nth(2)).not.toHaveAttribute('open', '')
  ...
  await persistedWitness.locator(':scope > summary').click()
